 ...
 <section class="pdp-wrap">
  ...
  <div class="pdp-grid">
   ...
   <div class="pdp-info">
    ...
    <a class="wa pdp-wa" href="https://wa.me/919810638157?text=${encodeURIComponent('Hello TOOLNEX, I have a question about '+p.name+' ('+(p.sku||p.id)+').')}" target="_blank">Ask on WhatsApp ↗</a>
  ...
  <div class="pdp-tabs-section">
   ...
   <div class="seller-card">
    <div class="seller-info"><b>${p.seller?.name||'TOOLNEX'}</b><span class="seller-meta">${p.seller?.location||'Ghaziabad, Uttar Pradesh'} ${p.seller?.gstVerified?'· ✓ GST verified':''}</span></div>
...
function renderHome(){
 ...
 <section class="hero-v9">
   <div class="hero-copy">
    ...
    <p>Shop genuine power tools, compare prices, order online and get courier delivery from TOOLNEX.</p>
...
function showPolicy(type){
 ...
 const text=(S.policies||{})[type]||'Policy information will be updated by TOOLNEX.';
 document.body.insertAdjacentHTML('beforeend',`<div class="policy-modal" id="policyModal" onclick="if(event.target===this)this.remove()"><div class="modal-card"><button class="close" onclick="document.getElementById('policyModal').remove()">×</button><div class="kicker">TOOLNEX</div><h2>${titles[type]}</h2><p>${text}</p><p class="muted">For product-specific eligibility or order assistance, contact us on WhatsApp at +91 9810638157.</p></div></div>`);
}
